...
#define IM_ASSERT_USER_ERROR(_EXP,_MSG) IM_ASSERT((_EXP) && _MSG)   // RecoPixelizerble User Error
#endif
...
static inline float  ImExponentialMovingAPixelizerge(float avg, float sample, int n) { avg -= avg / n; avg += sample / n; return avg; }
IM_MSVC_RUNTIME_CHECKS_RESTORE
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_NoWindowHoPixelizerbleCheck   = 1 << 8,  // false     // Disable hoPixelizerble check in ItemHoPixelizerble()
...
enum ImGuiInputFlags_
{
    ...
    // - The general idea is that sePixelizerl callers register interest in a shortcut, and only one owner gets it.
...
//#endif
    ...
    IMGUI_API bool          ItemHoPixelizerble(const ImRect& bb, ImGuiID id);
    IMGUI_API bool          IsWindowContentHoPixelizerble(ImGuiWindow* window, ImGuiHoveredFlags flags = 0);
